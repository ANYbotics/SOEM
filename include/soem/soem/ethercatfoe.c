            ...
            if ((aFOEp->MbxHeader.mbxtype & 0x0f) == ECT_MBXT_FOE)
            {
               switch (aFOEp->OpCode)
               {
                  ...
                  case ECT_FOE_BUSY:
                  {
                     ...
                     if (sendpacket)
                     {
                        // Not necessary with https://github.com/OpenEtherCATsociety/SOEM/pull/56
                        // if (!psize)
                        // {
                        //    dofinalzero = TRUE;
                        // }
                        ...
                        p = (uint8 *)p - segmentdata;
                        --sendpacket;
                        // Necessary for Maxon EPOS4 firmware update.
                        // Start from https://github.com/OpenEtherCATsociety/SOEM/pull/56
                        tsize = psize;
                        if (tsize > maxdata)
                        {
                           tsize = maxdata;
                        ...
                        // Stop from https://github.com/OpenEtherCATsociety/SOEM/pull/56
                     }
                     break;
                  }
                  case ECT_FOE_ERROR:
